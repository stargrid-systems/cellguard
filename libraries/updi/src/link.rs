...
///
/// A concrete target implements this over a USART in one-wire mode. Because the
/// line is shared, every byte the host sends is echoed back. [`UpdiLink::send`]
/// must consume that echo, so [`UpdiLink::recv`] returns only target bytes.
pub trait UpdiLink {
    /// Transport error.
    type Error;
...
#[cfg(test)]
mod tests {
    use super::{ACK, Updi, UpdiError, UpdiLink, cs};
