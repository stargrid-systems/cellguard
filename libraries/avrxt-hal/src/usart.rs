...
impl<T: UsartInstance> Usart<T> {
    ...
    ///
    /// Drains any in-flight frame first, then rewrites `CTRLC`/`CTRLB`. This
    /// lets one USART switch between, for example, an 8N1 command link and an
    /// 8E2 UPDI one-wire link on the fly.
