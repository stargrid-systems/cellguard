...
//!
//! [`RstInstance`] reads the reset-cause flags and triggers a software reset.
//! The flag set and the `SWRR` register are identical across AVR128 DA/DB and
//! tinyAVR 0/1-series: six read-only cause flags in `RSTFR` (cleared by writing
//! `1`), and an IOREG-protected `SWRR` that resets the chip when written.
//!
//! The software reset disables interrupts, opens the IOREG window on `CPU.CCP`,
//! then writes `SWRR`. It never returns.
